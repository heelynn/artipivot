...
**一个实例承载多条业务线**——每个业务线对应一个主 Agent，各自拥有独立的意图体系、子 Agent 组合、模型配置和记忆空间。业务线之间完全隔离，互不干扰。Web 面板管理配置，改完即时生效。CLI 一行启动，API 全开放。
...
**ArtiPivot 在 LangGraph 之上提供了一套完整的生产级 Agent 基础设施**——你不需要从零搭建路由、记忆、存储、容错和运维面板。

| 亮点 | 说明 |
|------|------|
| **意图路由开箱即用** | 内置 LLM 意图分类节点，支持 few-shot JSON prompt、置信度阈值门控、意图描述自动注入。路由规则运行时热更新，无需重建图。不用手写一个 classify 节点。 |
| **Multi-Agent 网关** | 一个实例承载多条业务线，每条业务线 = 一个主 Agent。业务线之间图、意图、子 Agent、记忆、模型配置完全隔离。子 Agent 跨业务线共享复用，相同定义自动去重。 |
| **三级持久记忆** | L1 工作记忆（请求级）→ L2 会话记忆（Checkpointer 快照）→ L3 长期记忆（LLM 自动提取画像/知识 + 语义检索）。存储后端可插拔：SQLite / PostgreSQL / MongoDB。 |
| **Web 管理面板** | 5 页 React SPA 覆盖日常运维——Agent 注册编辑、工具管理、路由配置、提示词编辑、熔断器调节、记忆策略、实时对话。改配置点保存即生效，零 YAML 手写。 |
| **原子热替换** | 修改子 Agent / 工具 / 路由 → 自动重建受影响的 Agent 图 → 原子替换。旧图继续服务存量请求，新请求走新图，构建失败不影响线上。 |
| **Graph DSL** | YAML 定义任意图拓扑——多段 LLM 接力、HITL 人工审批、条件分支、节点级重试、单节点多模型。零 Python 代码。 |
| **双状态架构** | `ArtiPivotState`（编排层）与 `SubAgentState`（执行层）分离，通过 LangGraph subgraph 边界映射，非引用传递。 |
| **原生容错** | 熔断器（per-provider 三级状态机）、指数退避重试 + 抖动、多维限流（用户/Agent/工具）——全部内置，per-agent 可配置。 |
| **分布式就绪** | PostgreSQL / MongoDB 存储后端使多实例共享状态成为可能。轮询式 ChangeNotifier 支持多进程配置同步。SSE 流式响应原生支持。 |

## 为什么需要 ArtiPivot

在 LangGraph 上从零搭建生产级 Agent 系统，你需要自己解决以下问题——ArtiPivot 已经内置：

| 你要自己写的 | ArtiPivot 的解决方式 |
|------------|---------------------|
| 意图分类节点（prompt + JSON 解析 + 阈值门控 + 澄清兜底） | `classify` 节点内置，`{intents}` 占位符自动注入路由表，置信度阈值和澄清话术 per-agent 可配置 |
| 多业务线 Agent 管理和隔离 | `AgentGateway` 按 `agent_id` 分发，每条业务线独立运行，五个维度（图/意图/子 Agent/记忆/模型）完全隔离，互不干扰 |
| 子 Agent 共享和去重 | `SubAgentRegistry` — 相同策略+工具 = 同一对象；私有子 Agent 自动命名空间隔离 |
| 存储层搭建（Checkpointer + Store + 配置存储 + 消息广播） | `StorageProvider` 一行切换 SQLite / PostgreSQL / MongoDB，四个后端统一管理 |
| 运行时配置热更新 | 模型、提示词、路由规则、限流 → 即时生效；工具/子 Agent 变更 → 自动重建受影响图 |
| 管理面板 | Web 页面注册 Agent、配置路由、编辑提示词、调节熔断器、管理记忆——点保存即生效 |
| 容错机制 | 熔断器 + 重试 + 限流全部内置，per-agent 独立配置 |
| 可观测性 | structlog + trace_id 全链路追踪、OpenTelemetry 指标导出、双日志文件（全量 + 错误告警） |
...
**意图分类**：LLM 读取路由表中每个 intent 的 `description`（通过 Web 页面或 API 配置），输出 `{intent, confidence}`。description 自动注入 classify prompt——LLM 理解意图边界，而不是靠名字猜。
