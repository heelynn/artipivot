...
**核心交付**：
1. 两种策略图（ReAct / Function Calling），各自独立的图拓扑
...
| Step 14: ~~CoT 策略~~ (已移除) | — | — | — | ReAct 是 CoT 的超集，已合并 |
...
CoT 策略已移除。ReAct 是 CoT 的超集（think → tools → think 循环已覆盖 plan → execute → synthesize 线性链的能力），因此 CoT 作为独立策略不再保留。
