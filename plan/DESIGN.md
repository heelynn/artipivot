...
| **高度可配置化** | 所有运行时参数存储在可配置后端（MongoDB / PostgreSQL / Redis），通过 API 管理 | 修改模型、调整限流不用改代码、不用重启服务 |
...
flowchart TD
    ...
    Declarative --> Store["写入 DocumentStore<br/>全集群可用"]
...
YAML 文件定义子代理的身份、意图、工具需求和参数。模型配置首次启动后由 DocumentStore 管理，运行时变更无需重启。
...
# 注：模型配置首次启动后由 DocumentStore 管理，通过 REST API 动态更新
# 运行时变更无需重启，无需重建图
```

#### 4.3.3 模型配置层级

每个子代理可以独立配置 LLM（大语言模型），支持**三级兜底**——主模型挂了自动切换备选，备选也挂了还有全局兜底。模型配置存储在 DocumentStore（可配置后端），通过 REST API 动态管理，变更立即生效（详见 [ARCHITECTURE.md](./ARCHITECTURE.md) 第 9 节）。

| 层级 | 配置位置 | 作用 | 管理方式 |
|------|----------|------|----------|
| 子代理模型 | DocumentStore → `model_configs` 集合 | 该子代理专用模型 | `PUT /admin/models/{agent_id}/{sub_agent}` |
| 子代理兜底 | 同上 → `fallback` 字段 | 子代理模型失败时降级 | 同上 |
| 全局兜底 | DocumentStore → `global` | 所有兜底都失败时的最后防线 | `PUT /admin/models/global/fallback` |

首次启动时从 YAML seed 文件加载初始配置到 DocumentStore，之后所有变更通过 REST API 管理。
...
支持三种注册方式，注册后框架自动完成：解析配置 → 写入 DocumentStore → ChangeNotifier 广播 → 各节点加载 → 路由表更新。
...
flowchart TD
    ...
    Validate --> Store["写入 DocumentStore"]
    Store --> Stream["ChangeNotifier 广播"]
...
> **这一节讲什么**：生产环境下，ArtiPivot 以多节点集群方式运行。所有插件（子代理/工具）的元数据通过 **DocumentStore**（可插拔后端）持久化，各节点通过 **ChangeNotifier**（可插拔通知机制）自动同步。本地开发时使用内存实现（零外部依赖），生产环境可选择 MongoDB、PostgreSQL 等后端。本节介绍集群架构、数据模型、插件热更新流程。
...
- **持久化层**：DocumentStore（可插拔后端，存储插件元数据 + 配置）、ChangeNotifier（可插拔通知，集群同步）、Redis（本地缓存 + 分布式锁）、ArtifactStore（可插拔制品存储，如 S3 / GCS / 本地文件系统）
...
flowchart TB
    ...
    subgraph Storage["持久化层（可插拔后端）"]
        DocStore[("DocumentStore<br/>插件注册表 + 配置<br/>MongoDB / PostgreSQL / Redis")]
        Notifier[("ChangeNotifier<br/>集群同步<br/>Change Stream / Pub/Sub")]
        Redis[("Redis<br/>本地缓存<br/>+ 分布式锁")]
        Artifact["ArtifactStore<br/>插件代码包<br/>S3 / GCS / 本地")]
    end
    ...
    N1 <-->|"ChangeNotifier<br/>实时同步"| DocStore
    N2 <-->|"ChangeNotifier<br/>实时同步"| DocStore
    N3 <-->|"ChangeNotifier<br/>实时同步"| DocStore
    ...
    style DocStore fill:#50C878,color:#fff,stroke:#2E8B57
    style Notifier fill:#50C878,color:#fff,stroke:#2E8B57
...
插件定义通过 **DocumentStore** 接口持久化。无论底层使用 MongoDB、PostgreSQL 还是内存，数据模型完全一致——三个核心"集合"（相当于三张表）：
...
**文档示例**（`plugins` 集合中的一条记录，具体存储格式由后端决定）：

```json
{
  "_id": "code_assistant_v1.2.0",
  "type": "sub_agent",
  "name": "code_assistant",
  "version": "1.2.0",
  ...
  "artifact_url": "artifacts://code_assistant-1.2.0-py3-none-any.whl",
...
下图展示了一个新插件从上传到全集群就绪的完整流程。关键步骤：管理员上传 → 存储制品 → 写入 DocumentStore → ChangeNotifier 触发各节点 → 节点下载安装 → 健康检查通过 → 开始接收流量。
...
节点启动时执行全量同步（从 DocumentStore 读取所有插件 → 下载安装 → 注册到本地缓存），然后启动 ChangeNotifier 监听，进入实时热更新循环。

```mermaid
flowchart TD
    Start["节点启动"] --> Boot["引导阶段"]
    Boot --> LoadCore["加载内置插件<br/>（Entry Points）"]
    LoadCore --> Connect["连接 DocumentStore + Redis"]
    ...
    Ready --> Watch["启动 ChangeNotifier 监听"]
    ...
    Watch -->|"定时"| HC["周期性健康检查<br/>上报到 DocumentStore"]
...
以下是集群级插件注册表的核心接口。它依赖三个可插拔抽象——`DocumentStore`（元数据存储）、`ChangeNotifier`（变更通知）、`ArtifactStore`（制品存储）——不再绑定任何具体后端。

```python
from abc import ABC, abstractmethod
from dataclasses import dataclass
from artipivot.storage import DocumentStore, ChangeNotifier, ArtifactStore
...
class ClusterPluginRegistry:
    """
    集群级插件注册表 — 依赖抽象接口，后端可插拔

    - 写操作 → DocumentStore（管理 API 调用）
    - 读操作 → 本地缓存（优先） → DocumentStore（兜底）
    - 热更新 → ChangeNotifier 订阅
    - 制品   → ArtifactStore 上传/下载
    """
    ...
    # --- 管理 API（写入 DocumentStore） ---

    async def publish_plugin(self, plugin_type: str, name: str,
                             artifact_path: str, manifest: dict) -> str:
        """发布插件：上传制品 + 写入 DocumentStore"""
        checksum = compute_sha256(artifact_path)
        artifact_url = await self._artifacts.upload(artifact_path, f"{name}/{manifest['version']}")
        doc = PluginDocument(
        ...
        # DocumentStore.put() 内部自动调用 notifier.notify()
        return await self._store.put("plugins", f"{name}:{doc.version}", doc.__dict__)

    async def deprecate_plugin(self, name: str) -> None:
        """标记插件为已弃用"""
        doc = await self._store.get("plugins", name)
        if doc:
            doc.data["status"] = "deprecated"
            await self._store.put("plugins", name, doc.data)
    ...
    async def get_sub_agent(self, intent: str) -> "BaseSubAgent":
        ...
        if cached and cached.is_healthy():
            return cached
        # 缓存未命中 → 从 DocumentStore 查询 + 实例化
        docs = await self._store.query("plugins", {"type": "sub_agent", "intent": intent})
        instance = await self._load_and_instantiate(docs[0])
    ...
    async def start_watcher(self) -> None:
        """订阅 ChangeNotifier，热更新本地插件"""
        await self._notifier.subscribe("plugins", self._on_plugin_change)
        await self._notifier.start()

    async def _on_plugin_change(self, collection, key, action, data):
        """收到变更通知后的处理"""
        await self._handle_change(action, key, data)

    async def _load_and_instantiate(self, doc) -> object:
        """下载制品 → 校验 → 安装 → 导入 → 实例化"""
        data = doc.data if hasattr(doc, 'data') else doc
        artifact_path = await self._artifacts.download(
            data["artifact_url"], tmp_path()
        )
        verify_checksum(artifact_path, data["checksum"])
        module = install_and_import(artifact_path, data["manifest"])
        return instantiate(module, data["manifest"])
```
...
| 2 | **DocumentStore + ArtifactStore** | 冷启动 / 缓存未命中 | 全量同步，ChangeNotifier 热更新 |
| 3 | **Entry Points（内置）** | 基础能力 | 内置插件随镜像发布，无需外部存储 |

下图展示了获取插件实例时的降级逻辑：Redis 缓存命中且健康 → 直接返回；否则查询 DocumentStore → 下载制品 → 安装实例化 → 写入缓存。
...
flowchart LR
    ...
    Check -->|是| Return["返回实例 ✅"]
    Check -->|否| Evict["驱逐缓存"]
    Cache -->|未命中| Store["查询 DocumentStore"]
    Evict --> Store
    Store --> Download["下载制品"]
...
| 审计 | `audit.log` + DocumentStore | 配置变更、插件操作、权限操作 | 365 天 |
...
| `CRITICAL` | 存储后端不可用、Checkpointer 不可用、磁盘满 |
...
| 基础设施 | DocumentStore/Redis/Postgres 连接池 + 重试 | — |
...
**容错参数动态配置**：超时和重试参数存储在 DocumentStore，通过 API 动态调整，无需重启服务。
...
> **这一节讲什么**：ArtiPivot 的所有运行时参数（模型、提示词、限流、路由规则）不写死在代码或配置文件中，而是存储在 **DocumentStore**（可配置后端），通过 REST API 动态管理。修改配置后通过 **ChangeNotifier** 立即生效。YAML 文件仅用于首次启动的初始数据导入。本地开发使用内存后端（零依赖），生产环境可选用 MongoDB、PostgreSQL 等。详细设计见 [ARCHITECTURE.md](./ARCHITECTURE.md) 第 10 节。

### 9.1 配置分类与生效方式

不同类型的配置有不同的生效方式——有些可以热更新（无需重启），有些需要重建图结构：

| 配置类型 | 存储位置（DocumentStore 集合） | 变更是否重建图 | 管理方式 |
|---|---|---|---|
| 模型配置 | `model_configs` | 否 — invoke 时动态解析 | `PUT /admin/models/...` |
| 提示词 | `prompt_configs` | 否 — 节点执行时读取 | `PUT /admin/prompts/...` |
| 限流参数 | `ratelimit_configs` | 否 — 中间件层拦截 | `PUT /admin/ratelimits/...` |
| 路由规则 | `routing_configs` | **是** — 条件边编译进图 | `PUT /admin/routing/...` |
| 子代理/工具 | `plugins` | **是** — 图拓扑变化 | 发布/下线 API |
...
存储后端通过 `artipivot.yaml` 配置，三个维度独立选择，可自由组合：

```yaml
# artipivot.yaml — 存储后端配置
storage:
  # 文档存储（插件元数据、配置、审计日志）
  document:
    backend: memory              # memory | mongodb | postgres | redis
    # mongodb:
    #   uri: mongodb://localhost:27017
    #   database: artipivot
    # postgres:
    #   uri: postgresql://localhost:5432/artipivot

  # 变更通知（集群同步）
  notifier:
    backend: memory              # memory | mongodb_stream | redis_pubsub | postgres_listen

  # 制品存储（插件代码包）
  artifacts:
    backend: local               # memory | local | s3 | gcs
    # s3:
    #   bucket: artipivot-plugins
    #   region: us-east-1
```

**典型组合**：

| 场景 | document | notifier | artifacts |
|------|----------|----------|-----------|
| 本地开发（零依赖） | `memory` | `memory` | `memory` |
| 单机测试 | `memory` | `memory` | `local` |
| 生产（MongoDB 全栈） | `mongodb` | `mongodb_stream` | `s3` |
| 生产（PostgreSQL + Redis） | `postgres` | `redis_pubsub` | `s3` |
...
│       │   └── provider.py
│       ├── storage/                  # 可插拔存储抽象层
│       │   ├── interfaces.py         # DocumentStore, ChangeNotifier, ArtifactStore
│       │   ├── bundle.py             # StorageBundle + create_storage() 工厂
│       │   ├── config.py             # StorageConfig
│       │   ├── document/             # DocumentStore 实现
│       │   │   ├── memory.py         # InMemoryDocumentStore（零依赖 dev）
│       │   │   ├── mongodb.py        # MongoDocumentStore（生产）
│       │   │   ├── postgres.py       # PostgresDocumentStore（生产）
│       │   │   └── redis.py          # RedisDocumentStore
│       │   ├── notifier/             # ChangeNotifier 实现
│       │   │   ├── memory.py         # InProcessNotifier（零依赖 dev）
│       │   │   ├── mongodb_stream.py # MongoChangeStreamNotifier
│       │   │   ├── redis_pubsub.py   # RedisPubSubNotifier
│       │   │   └── postgres_listen.py# PostgresListenNotifyNotifier
│       │   └── artifact/             # ArtifactStore 实现
│       │       ├── memory.py         # InMemoryArtifactStore（dev）
│       │       ├── local.py          # LocalArtifactStore（文件系统）
│       │       ├── s3.py             # S3ArtifactStore
│       │       └── gcs.py            # GCSArtifactStore
│       ├── config/                   # 动态配置中心
...
│       │   ├── audit.py              # 审计日志（文件 + DocumentStore）
│       │   └── otel.py               # OTel 可选导出
│       ├── resilience/               # 容错与弹性
...
| **可插拔存储** | `DocumentStore` + `ChangeNotifier` + `ArtifactStore` 三层抽象 | 存储后端、变更通知、制品存储全部可插拔；本地开发零依赖（内存实现），生产可选 MongoDB / PostgreSQL / Redis |
...
| 插件存储 | DocumentStore（可配置后端）+ ArtifactStore（可配置后端） | 后端可插拔，支持 MongoDB / PostgreSQL / Redis 等多种选择；ChangeNotifier 统一变更通知 |
...
| **动态配置** | ConfigCenter + DocumentStore + ChangeNotifier | 模型、提示词、限流等运行时参数全部动态管理，YAML 仅首次 seed；后端可插拔 |
...
### 第三阶段 — 集群 + 插件 + 可插拔存储 + 动态配置

实现生产级集群部署、插件热管理、可插拔存储后端、动态配置。

- [ ] DocumentStore 抽象接口 + InMemoryDocumentStore（零依赖 dev）
- [ ] ChangeNotifier 抽象接口 + InProcessNotifier（零依赖 dev）
- [ ] ArtifactStore 抽象接口 + LocalArtifactStore（文件系统）
- [ ] StorageBundle 工厂 + artipivot.yaml 配置解析
- [ ] MongoDB 后端实现（MongoDocumentStore + MongoChangeStreamNotifier）
- [ ] PostgreSQL 后端实现（PostgresDocumentStore + PostgresListenNotifyNotifier）
- [ ] 图热重建 + Gateway 原子替换
- [ ] 插件管理 REST API（发布/下线/灰度）
- [ ] Redis 本地缓存 + 分布式锁
- [ ] 健康检查 + 故障摘除
- [ ] ConfigCenter 动态配置（模型/提示词/限流/路由规则）
- [ ] DocumentStore 配置集合 + ChangeNotifier 热更新
...
- [ ] 审计日志（配置变更、插件操作 → DocumentStore）
- [ ] S3 / GCS ArtifactStore 实现
- [ ] MCP 协议适配器
